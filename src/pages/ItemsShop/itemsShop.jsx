...
export default memo(function ItemsShop() {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Ocultar skins ya compradas
    if (!inCollection && userSkins.length > 0) {
      const ownedIds = new Set(userSkins.map((s) => s.id));
      result = result.filter((item) => !ownedIds.has(item.id));
    }

    // Solo campeones que tienes
    if (championInCollection && userChampions.length > 0) {
      const ownedChampIds = new Set(userChampions.map((c) => c.id.toLowerCase()));
      result = result.filter((item) =>
        ownedChampIds.has(item.champion?.toLowerCase())
      );
    }
