...
export default memo(function Bag(){
	...
	<header className="bag-header">
		{sections.map( section => 
			<div className={`collection-tab ${actualSection === section ? 'active-collection-tab' : null}`} onClick={()=>{handleSound('menu-click'); setActualSection(section)}}>{section.toUpperCase()}</div>
		)}
	</header>
