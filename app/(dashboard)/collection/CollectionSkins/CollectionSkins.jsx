...
export default memo(function Pokemon(){
  ...
  const countRarity = userSkinsFull.reduce((acc, skin) => {
    ...
    fetch(`${API_URL}pokemons/data/skins`)
        ...
        .then(data=>{
            /*const dataFiltered = data.filter(d => d.availability === 'Available')*/
            setSkins(data);
            const userSkinsData = userSkins.map(us => {
            ...
            /*const groupedByYear = userSkinsData.reduce((acc, skin) => {
  ...
  function groupByChampionInitial(skins) {
    ...
    // ⬇️ siempre devolver array ordenado por inicial
  ...
  function getGroupedSkins(mode, showNotObtained, allSkins, userSkins) {
    ...
    // usamos un Map para acceder rápido a los no obtenidos por sección
    // eslint-disable-next-line no-undef
    ...
    // si quedan secciones solo de no obtenidos, las agregamos al final
  ...
  function applySectionSorting(grouped, groupedBy, sortedBy, userSkins) {
    ...
    switch (sortedBy) {
      case "purchaseDate":
        // Secciones = años de compra (descendente)
        sortedGrouped.sort(([a], [b]) => Number(b) - Number(a));
        break;

      case "releaseDate":
        // Secciones = años de lanzamiento (descendente)
        sortedGrouped.sort(([a], [b]) => Number(b) - Number(a));
        break;

      case "alphabetical":
        // Secciones = string (ej: campeones o sets)
        sortedGrouped.sort(([a], [b]) => a.localeCompare(b));
        break;

      case "mastery":
        // Campeones: se ordenan por maestría total
      ...
      case "mostOwned": {
        // Campeones o sets: se ordenan por cantidad de skins obtenidas
        // eslint-disable-next-line no-undef
        const obtainedIds = new Set(userSkins.map((s) => s.id));
        sortedGrouped.sort(([, skinsA], [, skinsB]) => {
      ...
      /*case "rarity":
        // "nivel" → no hay orden de secciones, se queda igual
        break;*/
  ...
  return <section className="collection-skins-section">
        <div className="skins-panel">
          <div className="left-place"> <div className="skins-panel-stats ">
          ...
          <div className="description">TOTAL DE ASPECTOS EN COLECCIÓN</div>
            ...
            <g id="Containers">
              <g>
                <g>
                  <path class="cls-1" d="M.5,6.47c3.31,0,6-2.67,6-5.97h157.3c0,3.3,2.69,5.97,6,5.97"/>
                  <path class="cls-1" d="M11.36,188.16c-1.55-4.48-5.82-7.71-10.86-7.71V12.19c5.04,0,9.31-3.22,10.86-7.71"/>
                  <path class="cls-1" d="M169.8,185.92c-3.31,0-6,2.67-6,5.97H6.5c0-3.3-2.69-5.97-6-5.97"/>
                  <path class="cls-1" d="M158.95,4.23c1.55,4.48,5.82,7.71,10.86,7.71v168.26c-5.04,0-9.31,3.22-10.86,7.71"/>
                </g>
              </g>
            </g>
          </svg>
        ...
        <div className="rarity-icons-container">
          <div className="rarity-icons">
            {raritys.map((rarity, index) => 
              <div key={index} className="rarity-item">
                <img key={index} className="rarity-image" src={`/raritys/${rarity}.png`}>
          ...
          <div className="right-place"> <div className="search-filter">
                <BsSearch className="search-icon" />
                <input placeholder="Buscar" type="search" onKeyUp={(event)=>setSearchKeys(event.currentTarget.value)}></input>
            </div>
            <div className="checkbox-container">{ groupedBy != 'collection' ? <div 
                    className="checkbox" 
