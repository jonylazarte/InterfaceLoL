...
export default memo(function MainPage(){
  ...
  const {loading, userChampions} = useSelector(selectUserPokemonData)
    ...
    const RenderChampsWithSections = () => {
        return Object.keys(renderData)?.map((section, index) => (
            <div key={index} className={`champion-section ${Object.keys(renderData).length -1 == index ? null :"border-b border-white/10" }`}>
                ...
                <main>
                    {renderData[section].length > 0 ? renderData[section]?.map((c, index) => (
                         <ToolTip key={index} delay={100} content={{'championName': c.name, 'maestryLevel': 1, 'masteryPoints': 0, 'maxSeasonRating': 'N/D', 'startInfo': 'A', 'eternals': ["Serie 1", "Serie 2", "Serie Inicial"], 'freeToPlay': false }} position="right">
                                <Card
                                    key={c.id || index} // Usar poke.id si está disponible, de lo contrario, index
                                    id={index}
                                    data={c}
